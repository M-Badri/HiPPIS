%function [yout, deg] = adaptiveinterpolation1D(x, y, xout, degree, interpolation_type, st, eps0, eps1, deg)
%%
...
%  %up_b = 0.0;%zeros(degree+1, 1);
%  %low_b = 0.0;%zeros(degree+1, 1);
%  %prod_deltax = 0.0; %zeros(degree+1, 1);
%  %lambda = 0.0;%zeros(degree+1, 1);
%  %sigma = zeros(degree+1, 1);
%  %prod_sigma = zeros(degree+1, 1);
%  %e = 0.0; %zeros(degree+1, 1);
%  %m_sigma = 100.0;
%  if (exist('deg'))
%    deg = zeros(n-1,1);
%  end
...
%      %%   if the interpolation_type variable is set to 1 **%%
%      mm_l(i) = 0.0;
%      mm_r(i) = 1.0;
