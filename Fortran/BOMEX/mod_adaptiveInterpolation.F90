...
subroutine adaptiveinterpolation1D_no_vec(x, y, n, xout, yout, m, degree, interpolation_type, st, eps0, eps1, deg ) 
!!
...
!!
  ...
  !!** interpolate along x **!!
  do j=1,ny
    call adaptiveInterpolation1D_no_vec(x, v(:,j), nx, xout, voutx(:,j), mx, degree, interpolation_type, sten, eps2, eps3)
  enddo

  !!** interpolate along y **!!
  do i=1,mx
    call adaptiveInterpolation1D_no_vec(y, voutx(i,:), ny, yout, vout(i,:), my, degree, interpolation_type, sten, eps2, eps3)
  enddo
  ...
  do k=1,nz
    do j=1,ny
      do ii=1, nx
        tmpin(ii) = v(ii,j,k)
      enddo
      call adaptiveInterpolation1D(x, tmpin(1:nx), nx, xout, tmpout(1:mx), mx, degree, interpolation_type, sten, eps2, eps3)
      do ii=1, mx
  ...
  do k=1,nz
    do j=1,ny
      do ii=1, nx
        tmpin(ii) = v(ii,j,k)
      enddo
      call adaptiveInterpolation1D_no_vec(x, tmpin(1:nx), nx, xout, tmpout(1:mx), mx, degree, interpolation_type, st, eps1, eps3)
      do ii=1, mx
        voutt(ii, j,k) = tmpout(ii)
      enddo
    enddo
  ...
  do k=1,nz
    do i=1,mx
      do jj=1, ny
        tmpin(jj) = voutt(i, jj, k) 
      enddo
      call adaptiveInterpolation1D_no_vec(y, tmpin(1:ny), ny, yout, tmpout(1:my), my, degree, interpolation_type, sten, eps2, eps3)
      do jj=1, my
        voutt(i, jj, k) = tmpout(jj)
      enddo
    enddo
  ...
  do j=1,my
    do i=1,mx
      do kk=1, nz
        tmpin(kk) = voutt(i,j, kk)
      enddo
      call adaptiveInterpolation1D_no_vec(z, tmpin(1:nz), nz, zout, tmpout(1:mz), mz, degree, interpolation_type, sten, eps2, eps3)
      do kk=1, mz
        vout(i, j, kk) = tmpout(kk)
      enddo
    enddo
