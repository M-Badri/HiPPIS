...
subroutine divdiff_no_vec(x, y, n, d, table)
!!! This subroutine computes the table of divided differences
!! 
!! INPUT:
!! x: 1D vector of x coorrdinates
...
subroutine adaptiveinterpolation1D_no_vec(x, y, n, xout, yout, m, degree, interpolation_type, &
                                         st, eps0, eps1, deg, & 
...
!!  m_lambda = 100.0              !! constant used to bound two consecutive \bar{lambda}_{j} and \bar{lambda}_{j+1}
  ...
  call divdiff_no_vec(x, y, n, degree+3, table)    !!compute the table of divided differences 
...
if(Debug == 1)then
  ...
  do j=2, degree
    ...
    elseif(stencil_type == 2) then
      ...
      !  write(*,*) 'i=', i, 'j=', j, 'st=2'
      !  write(*,*) 'b1(i)=', b1(i), 'b2(i)=', b2(i), 'b3(i)=', b3(i), 'b4(i)=', b4(i)
      !  write(*,*) 'b5(i)=', b5(i), 'b6(i)=', b6(i), 'b7(i)=', b7(i)
      !  write(*,*) 'bool(i)=', bool(i), 'bool2(i)=', bool2(i)
      !endif
      enddo
      !-OMP END SIMD
    ...
    do i=1, n-1
      ...
      !if(i==16 .and. j==3 .and. n==17 ) then
      ! write(*,*) 'i=', i, 'j=', j, 'n=17'
      ! write(*,*) 'bool(i)=', bool(i), 'bool2(i)=', bool2(i)
      ! write(*,*) 'tmp_ei=', tmp_ei, 'tmp_si=', tmp_si
      ! write(*,*) 'x(tmp_ei)=', x(tmp_ei), 'x(tmp_si)=', x(tmp_si)
      !endif
      !!
      !if(i==8 .and. j== 2) then
      !  write(*,*) 'i=', i, 'j=', j
      ...
      !if(i==32 .and. j == 6 .and. degree==8) then
      !  write(*,*) 'i=', i, 'j=', j, 'bool(i)=', bool(i), 'bool2(i)=', bool2(i)
      !  write(*,*) 'lambda_left(i)=', lambda_left(i), 'lambda_right(i)=',lambda_right(i)
      !  write(*,*) 'f_si(i)=', f_si(i), 'f_ei(i)=', f_ei(i)
      !  write(*,*) 'xx(i)=', xx(i)
      !endif
  ...
  do i=1,n-1
      ...
      ei = f_ei(i)
      !if(degree> 1)then
      !write(*,*) 'i=', i,  'degree=', degree, 'f_si(i)=', f_si(i), 'f_ei(i)=', f_ei(i)
      !endif
      !$OMP SIMD 
      do j=1, degree+1
        xval(j) = 0.0
      ...
      if( k <=m)then
        do while( x(i) <= xout(k) .and. xout(k) <= x(i+1) )
          call newtonPolyVal(xval, u, degree, xout(k), yout(k))
          !if(yout_debug(k) .ne. yout(k) .and. (yout(k)-yout_debug(k))> eps_debug &
          !   .and. Debug == 1 )then
          !  write(*,*) 'k=', k, 'i=', i, 'degee=', degree
          !  write(*,*) 'si=', si, 'ei=', ei
          !  write(*,*) 'yout(k)', yout(k), 'yout_bebug(k)=', yout_debug(k)
          !  write(*,*) 'u=', u
          !  write(*,*) 'xval=', xval
          !  call exit(1)
          !endif
...
!! d(2,3): NOT FINISHED
  ...
  !!** interpolate along x **!!
  do j=1,ny
    call adaptiveInterpolation1D(x, v(:,j), nx, xout, voutx(:,j), mx, degree, interpolation_type, st, eps0, eps1)
  enddo

  !!** interpolate along y **!!
  do i=1,mx
    call adaptiveInterpolation1D(y, voutx(i,:), ny, yout, vout(i,:), my, degree, interpolation_type, st, eps0, eps1)
  enddo


end subroutine

subroutine adaptiveInterpolation2D_no_vec(x, y, nx, ny, v,  xout, yout, mx, my, vout, degree, interpolation_type, st, eps0, eps1)
!!!This routine adaptively build interpolants that are the evaluated at xout, yout
!!
!! INPUT: 
!! x: 1D array of x coodinates
...
!! d(2,3): NOT FINISHED
  ...
  !!** interpolate along x **!!
  do j=1,ny
    call adaptiveInterpolation1D_no_vec(x, v(:,j), nx, xout, voutx(:,j), mx, degree, interpolation_type, st, eps0, eps1)
  enddo

  !!** interpolate along y **!!
  do i=1,mx
    call adaptiveInterpolation1D_no_vec(y, voutx(i,:), ny, yout, vout(i,:), my, degree, interpolation_type, st, eps0, eps1)
  enddo
...
!! d(3,3)
  ...
  do k=1,nz
    do j=1,ny
      do ii=1, nx
        tmpin(ii) = v(ii,j,k)
      enddo
      call adaptiveInterpolation1D(x, tmpin(1:nx), nx, xout, tmpout(1:mx), mx, degree, interpolation_type, st, eps0, eps1)
      do ii=1, mx
  ...
  do k=1,nz
    do i=1,mx
      ...
      enddo
      call adaptiveInterpolation1D(y, tmpin(1:ny), ny, yout, tmpout(1:my), my, degree, interpolation_type, st, eps0, eps1)
      do jj=1, my
  ...
  do j=1,my
    do i=1,mx
      ...
      enddo
      call adaptiveInterpolation1D(z, tmpin(1:nz), nz, zout, tmpout(1:mz), mz, degree, interpolation_type, st, eps0, eps1)
      do kk=1, mz
...
subroutine adaptiveInterpolation3D_no_vec(x, y, z, nx, ny, nz, v,  xout, yout, zout, mx, my, mz, vout, degree, &
                                   interpolation_type, st, eps0, eps1)
!!!This routine adaptively build interpolants that are the evaluated at xout, yout
!!
!! INPUT: 
  ...
  do k=1,nz
    do j=1,ny
      do ii=1, nx
        tmpin(ii) = v(ii,j,k)
      enddo
      call adaptiveInterpolation1D_no_vec(x, tmpin(1:nx), nx, xout, tmpout(1:mx), mx, degree, interpolation_type, st, eps0, eps1)
      do ii=1, mx
        voutt(ii, j,k) = tmpout(ii)
      enddo
    enddo
  ...
  do k=1,nz
    do i=1,mx
      do jj=1, ny
        tmpin(jj) = voutt(i, jj, k) 
      enddo
      call adaptiveInterpolation1D_no_vec(y, tmpin(1:ny), ny, yout, tmpout(1:my), my, degree, interpolation_type, st, eps0, eps1)
      do jj=1, my
        voutt(i, jj, k) = tmpout(jj)
      enddo
    enddo
  ...
  do j=1,my
    do i=1,mx
      do kk=1, nz
        tmpin(kk) = voutt(i,j, kk)
      enddo
      call adaptiveInterpolation1D_no_vec(z, tmpin(1:nz), nz, zout, tmpout(1:mz), mz, degree, interpolation_type, st, eps0, eps1)
      do kk=1, mz
        vout(i, j, kk) = tmpout(kk)
      enddo
    enddo
